...
export class InMemoryLearningFeedbackRepository implements LearningFeedbackRepository {
  ...
  async listEvents(id: string, tenantId: string) { const record = this.records.get(id); return record?.tenantId === tenantId ? structuredClone(this.events.get(id) ?? []).sort((left, right) => left.occurredAt.localeCompare(right.occurredAt) || left.id.localeCompare(right.id)) : []; }
