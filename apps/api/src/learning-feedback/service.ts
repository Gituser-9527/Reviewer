...
export class InMemoryLearningFeedbackRepository implements LearningFeedbackRepository {
  ...
  private async retention(input: RetentionOperationInput, execute: boolean): Promise<LearningFeedbackRetentionSummary> { const operationStartedAt = new Date(); const tenantRecords = [...this.records.values()].filter((record) => record.tenantId === input.tenantId); const anomalies = tenantRecords.filter((record) => classifyLearningFeedbackRetention(record, input.cutoff) === 'ANOMALY'); if (execute && anomalies.length > 0) return { runId: input.runId, tenantId: input.tenantId, mode: 'EXECUTE', status: 'ANOMALY', failureCode: 'GOLD_SET_ANOMALY', cutoff: input.cutoff.toISOString(), operationStartedAt: operationStartedAt.toISOString(), batchLimit: input.batchLimit, candidateCount: 0, deletedCount: 0, countsByStatus: {}, candidateIds: [], anomalyCount: anomalies.length, occurredAt: input.occurredAt.toISOString() }; const candidates = tenantRecords.filter((record) => new Date(record.updatedAt) < operationStartedAt && classifyLearningFeedbackRetention(record, input.cutoff) === 'CANDIDATE').sort((a, b) => a.id.localeCompare(b.id)).slice(0, input.batchLimit); const countsByStatus: Partial<Record<LearningFeedbackStatus, number>> = {}; for (const record of candidates) countsByStatus[record.status] = (countsByStatus[record.status] ?? 0) + 1; if (execute) for (const record of candidates) { this.records.delete(record.id); this.events.delete(record.id); } return { runId: input.runId, tenantId: input.tenantId, mode: execute ? 'EXECUTE' : 'DRY_RUN', status: 'SUCCEEDED', cutoff: input.cutoff.toISOString(), operationStartedAt: operationStartedAt.toISOString(), batchLimit: input.batchLimit, candidateCount: candidates.length, deletedCount: execute ? candidates.length : 0, countsByStatus, candidateIds: candidates.map((record) => record.id), anomalyCount: anomalies.length, occurredAt: input.occurredAt.toISOString() }; }
}
