...
export function registerLearningFeedbackRoutes(app: FastifyInstance, dependencies: { reviewStore: HumanReviewStore; service: LearningFeedbackService; authServices: AuthServices }): void {
  ...
  app.post('/api/reviews/:id/learning-feedback', async (request, reply) => {
    ...
    if (ticket === undefined) return reply.code(404).send({ requestId: request.id, error: { code: 'REVIEW_NOT_FOUND', message: 'Review ticket was not found.', retryable: false } });
    dependencies.authServices.authService.requireTenantAccess(request, ticket.tenantId);
    const record = await dependencies.service.submit(ticket, actor.userId, body);
    dependencies.authServices.auditLogService.record({ actor, operation: 'learning_feedback_submitted', resourceType: 'learning_feedback', resourceId: record.id, tenantId: ticket.tenantId, after: { status: record.status, digest: record.digest }, requestId: request.id });
  ...
  app.post('/api/learning-feedback/:id/withdraw', async (request, reply) => {
    ...
    if (actor.tenantId === undefined) return reply.code(400).send({ requestId: request.id, error: { code: 'TENANT_SCOPE_REQUIRED', message: 'Tenant scope is required.', retryable: false } });
    return reply.send(await dependencies.service.withdraw(params.id, actor.tenantId));
  });
  app.post('/api/learning-feedback/:id/review', async (request, reply) => {
    ...
    if (actor.tenantId === undefined) return reply.code(400).send({ requestId: request.id, error: { code: 'TENANT_SCOPE_REQUIRED', message: 'Tenant scope is required.', retryable: false } });
    return reply.send(await dependencies.service.review(params.id, actor.tenantId, body.status));
  });
  app.post('/api/learning-feedback/:id/promote-to-gold-set', async (request, reply) => reply.code(409).send({ requestId: request.id, error: { code: 'GOLD_SET_PROMOTION_UNAVAILABLE', message: 'Gold-set promotion is not implemented.', retryable: false } }));
}
