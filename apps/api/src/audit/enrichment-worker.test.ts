...
describe('EnrichmentWorker', () => {
  it('completes a mock explanation and records mock usage', async () => {
    ...
    const worker=new EnrichmentWorker({releaseExpiredLocks:async()=>0,claimNext:async()=>job,completeJob:async()=>{events.push('complete');},retryJob:async()=>{events.push('retry');},deadLetter:async()=>{events.push('dead');},createUsage:async input=>{expect(input.isMock).toBe(true);events.push('usage');}});
