...
export const learningFeedbackEvents = pgTable(
  ...
  (table) => [
    ...
    check('learning_feedback_events_reason_code_check', sql`${table.reasonCode} IS NULL OR ${table.reasonCode} IN ('QUALITY_VALIDATED','INSUFFICIENT_QUALITY','PRIVACY_CONCERN','OUT_OF_SCOPE','OTHER')`),
...
export const learningFeedbackRetentionRuns = pgTable(
  'learning_feedback_retention_runs',
  {
    ...
    failureCode: text('failure_code').$type<LearningFeedbackRetentionSummary['failureCode']>(),
