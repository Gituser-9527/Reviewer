...
浏览器扩展作为独立应用，以用户主动触发的 `ASSIST_MODE` 为唯一可用模式。它利用 `activeTab` 临时读取当前职位页，先在本地预览结构化 `WebJobCapture`，只有用户点击开始审核后才由后端保存脱敏快照并调用既有审核管线。采集 Adapter、页面定位和高亮与审核领域逻辑解耦；高亮仅用于辅助阅读。

未来 `SEMI_ACTIVE_MODE` 和 `ACTIVE_MODE` 必须通过单独 ADR 后实施，具备站点白名单、确认机制、限速、去重、状态验证、Pause/Stop、Kill Switch 和追加操作审计；不得默认执行最终处罚。
