...
5. 初始夹具会确定性触发脱敏 YAML 规则证据；手动高亮后应显示实际匹配数，并在页面中标出与 Finding 对应的文本。若显示 `0` 项，视为失败，不得当作高亮成功。
6. 手动清除高亮，确认全部扩展高亮标记消失。
7. 使用夹具页面按钮手动触发岗位身份变化。
8. 确认旧结果为 `STALE`，提交和高亮被禁用。
9. 重新加载有效岗位，确认不会自动提取、自动审核或自动高亮。
10. 仅在完成后回到终端输入 `DONE`。

## PASS 条件

- 人工提取、修正、提交、Finding/Evidence 查看、高亮与清除均成功；
- 初始脱敏夹具至少产生一个可定位 Evidence；若结果没有可定位 Evidence，切换或替换夹具前必须停止并记录为 Smoke 失败；
- `STALE` 安全保护生效；
- 不发生自动审核或自动高亮；
