...
API 使用显式 tenant 认证与精确 extension Origin CORS；没有 wildcard 或 cookie credentials。AuditRun、Findings、Evidence 按 tenant 持久化，页面变化不会删除后端历史。日志和反馈必须脱敏；附件只能存放在批准的位置，并由维护人员按测试环境清理责任处理。
